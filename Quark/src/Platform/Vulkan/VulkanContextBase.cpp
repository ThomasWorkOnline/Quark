...
namespace Quark {
	...
	SwapSurfaceFormat VulkanContextBase::ChooseSurfaceFormat(SwapSurfaceFormat preferred) const
	{
	...
	SwapPresentMode VulkanContextBase::ChooseSwapPresentMode(SwapPresentMode preferred) const
	{
	...
	SwapExtent VulkanContextBase::ChooseSwapExtent(uint32_t width, uint32_t height) const
	{
		auto& capabilities = m_Device->GetSupportDetails().Capabilities;
		if (capabilities.currentExtent.width != std::numeric_limits<uint32_t>::max())
		{
			return SwapExtent{ capabilities.currentExtent.width, capabilities.currentExtent.height };
		}
		else
		{
			SwapExtent extent = { width, height };
