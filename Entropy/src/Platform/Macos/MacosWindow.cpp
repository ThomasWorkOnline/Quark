...
namespace Entropy {
	...
	MacosWindow::MacosWindow(uint32_t width, uint32_t height, const char* title)
	{
	...
	void MacosWindow::Init(uint32_t width, uint32_t height, const char* title)
	{
		m_Data.Title = title;
		m_Data.Width = width;
	...
	const std::string& MacosWindow::GetTitle()
	{
		return m_Data.Title;
	}
	...
	void MacosWindow::AppendTitle(const std::string& title)
	{
		SetTitle(m_Data.Title + title);
	}

	void MacosWindow::Select()
	{
		glfwSetInputMode(m_Window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
	}

	void MacosWindow::Deselect()
	{
		glfwSetInputMode(m_Window, GLFW_CURSOR, GLFW_CURSOR_NORMAL);
	}

	bool MacosWindow::IsCursorDisabled() const
	{
		return glfwGetInputMode(m_Window, GLFW_CURSOR) == GLFW_CURSOR_DISABLED;
	}

	bool MacosWindow::IsCursorNormal() const
	{
